...
print("="*70)
print("BACKTEST: 100k Sharpe Model")
print("="*70)
# Load data - use 2023 (unseen year) as test
df = pd.read_csv("/lustrefs/project/25sfcs03/FIRST_TEST/data/cached/PTT_BK_2020-01-01_2024-01-01.csv", 
                 index_col=0, parse_dates=True)
# Different split: use 2023 as "unseen" test data
test_df = df[df.index.year == 2023]
print(f"\nTest period: {test_df.index[0]} to {test_df.index[-1]}")
print(f"Test days: {len(test_df)}")
print(f"Price: {test_df['Close'].iloc[0]:.2f} → {test_df['Close'].iloc[-1]:.2f}")
# Load 100k model
model = PPO.load("/lustrefs/project/25sfcs03/FIRST_TEST/checkpoints/ptt_sharpe_100k")
...
if sells > 0:
    print("\n🎉 AGENT LEARNED TO SELL!")
elif buys > 0:
    print("\n⚠️ Still only buying, no selling")
else:
    print("\n❌ Back to holding only")
print("="*70)
